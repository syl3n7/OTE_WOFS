...
public class CountourScanner : WebCamera
{
    ...
    protected override bool ProcessTexture(WebCamTexture input, ref Texture2D output)                             
    {
        ...
        CascadeClassifier handCascade = new CascadeClassifier("Assets/rpalm.xml");
        Mat frame = OpenCvSharp.Unity.TextureToMat(input);
        //Mat gray = new Mat();
        //Cv2.CvtColor(frame, gray, ColorConversionCodes.BGR2GRAY);
        Rect[] hands = handCascade.DetectMultiScale(frame, 1.1, 2, HaarDetectionType.ScaleImage, new Size(30, 30));

        if (hands.Length > 0)
        {
            //Vec3b color = frame.At<Vec3b>(hands[0].Y + hands[0].Height / 2, hands[0].X + hands[0].Width / 2);
            //Debug.Log(color.Item2);
            //if (color.Item2 > 10) // Check if color is yellow
            //{
