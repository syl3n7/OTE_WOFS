...
Menu(float x, float y) {
    ...
    background = new Background("assets/images/Background_MainMenu.png", width/2, height/2);//background do menu
    start = new Button("assets/images/Play_Button.png", width/2 - 350, height/2 - 100);//botao para comecar o jogo
    ...
    void start() { //desenhar e verificar estado dos botões
        ...
        /*
         ? The following section of code handles the button presses and updates the state of the menu.
         * It checks for button presses on the instructions button, back button, highscore button, and highscore back button.
         * If the instructions button is pressed, it sets the state to false and activates the instructions screen.
         * If the back button on the instructions screen is pressed, it deactivates the instructions screen and sets the state to true.
         * If the highscore button is pressed, it sets the state to false and activates the highscore screen.
         * If the back button on the highscore screen is pressed, it deactivates the highscore screen and sets the state to true.
         */
        if(state){  //verficar estado pressed de cada botao
            if (start.pressed) 
            {
                background.changeImage("assets/images/background1080p.png");
                displayGame = true;  //se o botao start nao estiver a ser pressionado entao o menu continua a ser mostrado.
            }
        ...
        if(back.pressed) {
            ...
            start.pressed = false;
            background.changeImage("assets/images/Background_MainMenu.png"); 
            back.pressed = false;
        }
